...
<?php
/*Статистика*/
$SELECT_USERS = implode(", ",$USER->family_users);
...
$sql = sprintf("SELECT SUM(`cost`) as costs, `line_id` FROM `outgo_data` WHERE `time` >= %s AND `time` < %s AND `user_id` IN (%s) AND `line_id` != 5 GROUP BY `line_id` ORDER BY costs DESC",
...
if(count($data) > 0){
	...
	foreach($data as $line=>$val){
		...
		$stat_line[] = sprintf("['<span onclick=\"view_detail(\'%s\',\'summary\')\">%s(%s)</span>',%s]",$line_val,$line_name,$val,$val);
	}
	...
	implode("",$select));
	if($summary){
		$my_group = $USER->get_family_users();
		$sql = sprintf("SELECT * FROM `outgo_data` WHERE `time` >= %s AND `time` < %s AND `user_id` IN (%s) AND `line_id` IN (%s) ORDER BY `time` DESC",
